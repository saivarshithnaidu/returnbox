...
export default function AddProductPage() {
  ...
  const [form, setForm] = useState({ name: '', slug: '', description: '', short_description: '', category_id: '', price: '', sale_price: '', stock_count: '0', is_featured: false, is_active: true, fragrance: '', materials: '', burn_time_hours: '', weight_grams: '', dimensions: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const body = { ...form, slug: form.slug || slugify(form.name), price: parseFloat(form.price), sale_price: form.sale_price ? parseFloat(form.sale_price) : null, stock_count: parseInt(form.stock_count), burn_time_hours: form.burn_time_hours ? parseInt(form.burn_time_hours) : null, weight_grams: form.weight_grams ? parseInt(form.weight_grams) : null, category_id: form.category_id || null, images };
      const res = await fetch('/api/admin/products', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      if (res.ok) { toast.success('Product created!'); router.push('/admin/products'); } else toast.error('Failed');
    } catch { toast.error('Error'); }
    setLoading(false);
  };

  const u = (k: string, v: any) => { setForm(f => ({ ...f, [k]: v })); if (k === 'name' && !form.slug) setForm(f => ({ ...f, slug: slugify(v) })); };
  ...
  return (
    <div className="max-w-3xl space-y-6">
      <h1 className="font-serif text-2xl text-white">Add Product</h1>
      <form onSubmit={handleSubmit} className="bg-[#1A1010] rounded-xl border border-white/5 p-6 space-y-5">
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div><label className="font-sans text-xs text-[#8B5E5E] mb-1 block">URL Slug</label><input className={inputCls} value={form.slug} onChange={e => u('slug', e.target.value)} placeholder="auto-generated" /></div>
        </div>
        ...
        <button type="submit" disabled={loading} className="bg-[#B76E79] text-white px-6 py-3 rounded-xl font-sans font-semibold hover:bg-[#9a5a65] transition-colors flex items-center gap-2 disabled:opacity-50">
          {loading ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Save Product
        </button>
      </form>
    </div>
  );
